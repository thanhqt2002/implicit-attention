...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='',pretrained_cfg=None,pretrained_cfg_overlay=None,s_scalar=False, num_implicit_layers=1):
        """
        Args:
        ...
        if self.num_implicit_layers > -1:
            ...
            self.blocks = nn.Sequential(*([
                ...
                Block(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                    attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, layerth = i, 
                    ttl_tokens=num_patches+self.num_tokens,s_scalar=self.s_scalar, attention_type="explicit")
                for i in range(self.num_implicit_layers, depth)
                ]))
        else: # all layers are implicit
            print(f"Using {depth} implicit layers")
            self.blocks = nn.Sequential(*[
                Block(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                    attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, layerth = i, 
